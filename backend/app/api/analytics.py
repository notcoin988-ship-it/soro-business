...
ОЦЕНКИ КЛИЕНТОВ — пятибалльные, как обещает прототип («4,4/5 по 380
оценкам»). В исходном DDL шкала была из двух значений; её поменяла
миграция 0003, разбор — в шапке `core/feedback.py`.
"""
