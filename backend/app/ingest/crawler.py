...
Чего в ТЗ нет и что решено здесь:
...
3. **PDF и прочие не-HTML на сайте — пропускаем, но собираем список.**
   Скачивание, сохранение в volume и создание `documents` — работа
   `ingest/worker.py`, а не краулера. Найденные адреса отдаём в
   `CrawlResult.assets`, чтобы человек решил, грузить ли их руками через
   POST /api/documents.
...
13. **Потолок на размер страницы — 5 МБ.** Всё, что больше, это не
   ...
   защита требует потокового чтения.
14. **TLS проверяется по умолчанию, отключается только явно.** Прогон по
   ...
   но включается руками и пишет предупреждение в лог: без проверки
   сертификата содержимое сайта можно подменить по дороге, а мы кладём его
   в базу знаний банка и выдаём клиентам как официальный ответ.
"""
