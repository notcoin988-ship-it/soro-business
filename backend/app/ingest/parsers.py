...
# ФОРМЫ ДОБАВЛЕНЫ СВЕРХ ТЗ (19.08.2026). В ТЗ перечислены nav/header/
# footer, но на сайте банка сквозные формы заявок лежат ВНУТРИ <main> и
# потому попадали в текст каждой страницы: «Оставить заявку ФИО Телефон
# Я соглашаюсь на обработку персональных данных…», плюс выпадающие
# списки фильтров («Год 2024 2023 Месяц Январь Февраль…»).
#
# ЧЕМ ЭТО ПЛОХО. Такой текст одинаков на сотне страниц и семантически
# нейтрален — он всплывал в топе почти любого запроса и вытеснял
# настоящий контент. Живой прогон, вопрос «хочу оформить кредит»: все
# три фрагмента — бланки заявок, ответа в них нет, модель отвечала
# `[ESCALATE]`, хотя страницы «Кредит Многоцелевой» и «Ипотека» с
# суммами и ставками в базе есть.
#
# Замер на eskhata.com/loans/multi/: 3801 → 2338 символов. Условия
# кредита (сумма, срок, ставка 29% годовых) сохранились полностью —
# вырезаны только подписи полей ввода, которые для поиска бесполезны.
DROP_TAGS = (
    "nav",
    "header",
    "footer",
