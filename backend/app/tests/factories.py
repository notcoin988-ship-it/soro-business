...
Почему синтетика, а не реальные PDF банка: ТЗ требует «3 реальных PDF
банка» и обещает их от тимлида — их нет. Ждать нельзя, неделя сгорит,
поэтому фикстуры собираются здесь, а содержимое взято максимально близко к
настоящим тарифам: таджикский с диакритикой (ӣ ӯ ҳ ҷ ғ қ), русский,
проценты, суммы в сомонӣ, таблицы.

Файлы создаются во временной папке теста и в git не попадают: бинарники в
репозитории протухают, а генератор всегда собирает актуальную версию.
