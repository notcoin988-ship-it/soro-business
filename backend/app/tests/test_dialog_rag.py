...
def reachable_threshold(monkeypatch):
    """Порог опущен намеренно — иначе до модели дело не доходит.

    Прогон golden set показал: таджикский вопрос про «Ояндасоз» набирает
    ~0,63 при пороге ТЗ 0,65, то есть верный ответ отсекается ещё до
    вызова модели (см. ARCHITECTURE.md). Здесь проверяется связка «поиск →
    модель → ответ», а поведение самого порога — в `test_rag.py` и
    `test_dialog.py`. Не опустить его тут значит тестировать эскалацию
    вместо связки.
    """
