...
async def test_tajik_correct_answer_sits_near_tz_threshold(session, knowledge):
    """СТОРОЖ КАЛИБРОВКИ. Правильный таджикский ответ набирает ~0,63.

    Порог раздела 6.4 — 0,65. То есть на вопросе, ради которого строится
    всё демо, поиск находит верный фрагмент первым, но объявляет «ответа
    нет» и уходит в эскалацию. Ложная эскалация безопаснее выдумки, но на
    приёмке (шаг 2 сценария) это провал.

    Русские формулировки на той же базе дают 0,68 — дело не в поиске, а в
    том, что bge-m3 знает таджикский хуже, и все его оценки смещены вниз.
    Правится калибровкой порога по golden set (раздел 3.2, диапазон
    0,60–0,72) — это неделя 3 и `scripts/eval_rag.py`, менять константу
    ТЗ на глазок нельзя.

    Тест обязан упасть, когда порог откалибруют: это не поломка, а сигнал
    убрать сторож и обновить цифры здесь.
    """
    ...
    assert settings.RAG_MIN_SCORE == 0.65, "порог изменили — обновите этот тест"
    assert not result.has_answer, (
        "порог откалибровали и таджикский теперь проходит — удалите этот "
        "сторож и поправьте test_finds_target_fragment_in_tajik"
    )
