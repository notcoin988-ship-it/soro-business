...
#
# ПРАВИЛО 8 ДОБАВЛЕНО ПОЗЖЕ, когда появилась память диалога. Правило 7
# велит задавать уточняющий вопрос, клиент отвечает «бале» — и модель
# выдавала ТОТ ЖЕ абзац слово в слово, по три раза подряд. Она видит
# свой прошлый ответ в истории и, не найдя во фрагментах нового,
# пересказывает его.
#
# Слов «[ESCALATE]» в правиле нет намеренно: добавление маркера в
# правило уже приводило к тому, что модель лепила его на исправные
# ответы. Решение об эскалации принимает наш код (`context.is_repeat`),
# а не промпт.
...
8. НЕ ПОВТОРЯЙ то, что уже сказал в этом разговоре. Клиент это
...
6. Согласие («бале», «ҳа», «да») переписывай в ТУ ТЕМУ, которую бот
   предложил последним вопросом, и ТОЛЬКО в неё — прежнюю тему разговора
   в вопрос не тащи.
...
Пример 3.
История: Бот: Корти миллӣ-ро дар шуъба фармоиш диҳед. Оё мехоҳед
маълумоти бештар дар бораи барномаи мобилии «Эсхата Онлайн» гиред?
Реплика: бале
Ответ: маълумоти бештар дар бораи барномаи мобилии «Эсхата Онлайн»"""
